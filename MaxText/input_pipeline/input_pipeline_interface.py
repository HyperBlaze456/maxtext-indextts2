...
from MaxText.input_pipeline._emilia_audio_processing import make_emilia_audio_train_iterator, make_emilia_audio_eval_iterator
...
def create_data_iterator(config: pyconfig.HyperParameters, mesh):
  ...
  dataset_type_to_train_eval_iterator = {
      ...
      "emilia_audio": (make_emilia_audio_train_iterator, make_emilia_audio_eval_iterator),
  }

  # Collect train and eval iterators
  if config.dataset_type in ["tfds", "grain", "hf", "c4_mlperf", "emilia_audio"]:
    ...
    max_logging.log(
        ...
        "Using synthetic data. Please choose from 'tfds', 'grain', 'hf', 'c4_mlperf', or 'emilia_audio'."
    )
